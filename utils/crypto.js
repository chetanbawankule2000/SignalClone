...
export const stringToUTF8array = (ourSecretKeyString) =>
  Uint8Array.from(ourSecretKeyString.split(",").map((str) => parseInt(str)));

export const getSecretKey = async () => {
  const ourSecretKeyString = await AsyncStorage.getItem("Private_key");
  if (!ourSecretKeyString) {
    Alert.alert(
      "You havent set your key pairs yet",
  ...
  return stringToUTF8array(ourSecretKeyString);
};
